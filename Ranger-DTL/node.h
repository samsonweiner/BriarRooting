...
public:
	...
	// returns a vector of all descendants of a given node
	inline vector<T*> getDescendants() {
		vector<T*> nodes;
		T *children[2];
		getChildren(children);
		for (int i = 0; i < 2; i++) {
			getDescendantsDFS(children[i], nodes);
		}
		return nodes;
	}
	inline void getDescendantsDFS(T *&node, vector<T*> &nodes) {
		if (node == NULL) return;

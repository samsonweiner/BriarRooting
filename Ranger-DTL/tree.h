...
public:
	...
	// reroot a gene tree or species tree.
	
	void reroot(NODE *node)
	{
		// do nothing if rerooting at root node or a child of the root
		if (node == root)
			return;
		if (node->parent() == root)
			return;
		...
		// otherwise reroot step by step using recursion
//		if (node->parent()->parent() != root)
			reroot(node->parent());
		
//cout << "sec " << node << " " << node->parent()<< " " << node->parent()->parent() << endl;
//cout << "root: " << root << endl;
			
			NODE *a = node;
			NODE *ap = node->parent();
			NODE *app = node->parent()->parent();
			NODE *as = node->getSibling();
			NODE *aps = node->parent()->getSibling();
				
			if (app != root) // sanity check
			{
					cout << "ERROR in rerooting" << endl;
					exit(1);
			}

			if((node ==  node->parent()->child(0)) && (node->parent() == node->parent()->parent()->child(0)))
			{
				app->child(0) = a;
				app->child(1) = ap;
				a->parent() = app;
			...
			else if((node ==  node->parent()->child(1)) && (node->parent() == node->parent()->parent()->child(0)))
			{
				app->child(0) = ap;
				app->child(1) = a;
				a->parent() = app;
			...
			else if((node ==  node->parent()->child(0)) && (node->parent() == node->parent()->parent()->child(1)))
			{
			...
			else if((node ==  node->parent()->child(1)) && (node->parent() == node->parent()->parent()->child(1)))
			{
				app->child(0) = ap;
				app->child(1) = a;
				a->parent() = app;
...
// newly added function to display gene tree node indexes also.	
	public: void genetree2newick(ostream &os) {
		genesubtree2newickDFS(os, root);
		os << ';';
	}
	protected: void genesubtree2newickDFS(ostream &os, NODE *&node) {
		if (node == NULL) return;
		...
		if (!node->isLeaf()) {
			...
			if ((node->child(0)!=NULL) || (node->child(1)!=NULL)) os << '(';
			// proceed with the children nodes
			genesubtree2newickDFS(os, node->child(0));
			if (node->child(1)!=NULL) os << ',';
			genesubtree2newickDFS(os, node->child(1));
			// close the subtree
			if ((node->child(0)!=NULL) || (node->child(1)!=NULL)) os << ')' << 'm' << node->idx+1;
		}

...
public:
	...
	inline bool nextAnyChar(char &c) {
		while (nextChar(c)) {
			if ((skipComments) && (c=='[')) {
				...
				if (commentcallback!=NULL) commentcallback->commentFound(str);
			}
	...
	inline double readNumber() {
		...
		double value;
		ss >> value;
		if (ss.fail()) EXCEPTION("cannot read numeric value at " << getPos());

		// look for junk after the value
		string junk;
		ss >> junk;
		if (junk.size() != 0) EXCEPTION("cannot read numeric value at " << getPos());
		return value;
	}
